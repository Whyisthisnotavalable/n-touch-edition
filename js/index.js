...
//touch move input
let AD = false; //attack/defend
document.body.addEventListener("touchmove", (event) => {
	if(event.touches[0].clientX > window.innerWidth / 3) {
		simulation.mouse.x = event.touches[0].clientX;
		simulation.mouse.y = event.touches[0].clientY;
	} else {
		simulation.mouse.x = event.touches[1].clientX;
		simulation.mouse.y = event.touches[1].clientY;
	}
	if(event.touches[1] || event.touches[2]) {
		event.preventDefault();
	}
}, { passive: false });
...
let touch = []; //yes
touch.push(document.getElementById("up"));
touch.push(document.getElementById("down"));
touch.push(document.getElementById("left"));
touch.push(document.getElementById("right"));
touch.push(document.getElementById("change"));
touch.push(document.getElementById("switchDown"));
touch.push(document.getElementById("switchUp"));
touch[0].addEventListener("touchstart", (event) => {
	if (m.onGround == true) {
		input.up = true;
	}
});
touch[0].addEventListener("touchend", (event) => {
	input.up = false;
});
touch[1].addEventListener("touchstart", (event) => {
	if (m.onGround == true) {
		input.down = true;
	}
});
touch[1].addEventListener("touchend", (event) => {
	input.down = false;
});
touch[2].addEventListener("touchstart", (event) => {
	input.left = true;
});
touch[2].addEventListener("touchend", (event) => {
	input.left = false;
});
touch[3].addEventListener("touchstart", (event) => {
	input.right = true;
});
touch[3].addEventListener("touchend", (event) => {
	input.right = false;
});
touch[4].addEventListener("click", (event) => {
	if(AD == false) {
		AD = true;
		touch[4].style.backgroundColor = "rgba(255, 102, 102, 0.4)";
	} else {
		AD = false;
		touch[4].style.backgroundColor = "rgba(0, 255, 255, 0.4)";
	}
});
touch[5].addEventListener("click", (event) => {
	simulation.previousGun();
});
touch[6].addEventListener("click", (event) => {
	simulation.nextGun();
});
